...
@patch.object(
    SaltBase,
    "call_process",
    return_value={"retcode": 0, "stdout": b"salt-call 3004.1\r\n", "stderr": b""},
)
...
def test_pip_package_installed(saltworker_client):
    ...
    expected = ["pip.install", "dnspython", "index_url=https://pypi.org/simple"]
...
def test_windows_missing_prepdir():
    ...
    system_params["logdir"] = Path("logdir")
    system_params["workingdir"] = Path("workingdir")
...
def test_windows_missing_logdir():
    ...
    system_params["prepdir"] = Path("prepdir")
    system_params["workingdir"] = Path("workingdir")
...
def test_windows_defaults():
    ...
    system_params["prepdir"] = Path("8cbda638-6b60-5628-870b-40fdf8add9f8")
    system_params["logdir"] = Path("21fa57e2-9302-5934-978d-4ae40d5a2a55")
    system_params["workingdir"] = Path("c990ee27-ff12-5d7e-9957-1d27d114c0ff")
    ...
    assert win_salt.salt_base_env == win_salt.salt_srv / "states"
    assert win_salt.salt_formula_root == win_salt.salt_srv / "formulas"
    assert win_salt.salt_pillar_root == win_salt.salt_srv / "pillar"
    ...
    assert win_salt.salt_conf["pillar_roots"] == {
        "base": [str(win_salt.salt_srv / "pillar")],
    }
...
def test_windows_install(saltworker_base_salt_args):
    ...
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("0dcd877d-56cb-50c2-954a-80d1084b2216")
    system_params["logdir"] = Path("647c2a49-baf9-511b-a17a-d6ebf0edd91c")
    system_params["workingdir"] = Path("3d6ab2ef-09ad-59f1-a365-ee5f22c95c79")
...
def test_windows_salt_call_new():
    ...
    salt_path = Path("C:\\Program Files") / "Salt Project" / "Salt" / "salt-call.exe"
    assert SaltWindows._get_salt_call() == salt_path
...
def test_windows_prep_install(mock_safe, mock_mkdir, mock_open):
    """Ensure that prep portion of install runs as expected."""
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("ac2bf0c3-7985-569f-bfbd-3a8d8a13ce7d")
    system_params["logdir"] = Path("5b0976f8-fcbc-50af-9459-8060589e70d9")
    system_params["workingdir"] = Path("860f630a-f85d-5ed2-bd7a-bbdb48a53b2b")
    ...
    # Path.open is called as a method, so first arg is the path itself
    minion_path = Path(saltworker_win.salt_conf_path) / "minion"
    mock_open.assert_called_with(minion_path, "w", encoding="utf-8")
...
def test_linux_computer_name_none():
    ...
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("662f1bdb-5992-5f8f-87d6-15c4de958b7b")
    system_params["logdir"] = Path("76b74ceb-e81d-5fac-b293-0d7d45901ef7")
    system_params["workingdir"] = Path("4e6a1827-1d3b-5612-a7fd-f6fed00b5a2f")
...
@patch("pathlib.Path.mkdir", autospec=True)
@patch("yaml.safe_dump", autospec=True)
def test_linux_salt_debug_log_none(mock_safe, mock_mkdir, mock_open):
    """Test that Pythonic None can be used without error rather than 'None'."""
    # setup ========================
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("d6f194e9-7cbc-5c89-9c9c-7ad43faf8a7b")
    system_params["logdir"] = Path("9523029a-b7a9-50a1-83c8-d0a1b39f496f")
    system_params["workingdir"] = Path("f92163af-f801-5880-983a-aeb880c94a0b")
    ...
    # assertions ===================
    assert saltworker_lx.salt_debug_logfile == Path(salt_config["salt_debug_log"])
    ...
    # assertions ===================
    assert saltworker_lx.salt_debug_logfile == (
...
def test_linux_salt_content_none(
    ...
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("1868795c-7d47-5d4d-9021-941b449967af")
    system_params["logdir"] = Path("e88611fe-d444-5951-bb20-4d7508ed7a0c")
    system_params["workingdir"] = Path("f671935d-6669-50d1-a63c-157799cd13cb")
...
def test_linux_ou_path_none():
    ...
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("dba0f4db-3851-54b0-b2f7-bfff6bb8bbf5")
    system_params["logdir"] = Path("f9d62bf8-7ffe-58a8-9ad9-2d21a430e52b")
    system_params["workingdir"] = Path("0a5e5983-3d95-58b3-92a8-c4673902b47d")
...
def test_linux_admin_groups_none():
    ...
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("a27f71e5-4af3-5ad4-9d69-05fb8b7cbe4f")
    system_params["logdir"] = Path("9b504864-814c-5226-9a05-b7613dd96efa")
    system_params["workingdir"] = Path("d468d406-1e89-5041-9900-0a9189e541b0")
...
def test_linux_admin_users_none():
    ...
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("ff7b4ffb-99a0-50c2-af78-98ebfea33295")
    system_params["logdir"] = Path("3744bc20-0357-50f1-8633-178151d904ab")
    system_params["workingdir"] = Path("455feea8-481c-5014-8a7b-cdeb619a2177")
...
def test_win_ash_role_none():
    ...
    system_params = {}
    salt_config = {}
    system_params["prepdir"] = Path("4e91a26a-c325-5548-9e99-e0cea505eca6")
    system_params["logdir"] = Path("1a74aa66-1eb5-5f6e-bbdc-a247cfabdcb7")
    system_params["workingdir"] = Path("3cbd1221-01f3-5ed8-8df1-79918a1ebe3d")
