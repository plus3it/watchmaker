...
def test_config_w_status_provider():
    """Test config that has the status block and supported provider."""
    with patch("watchmaker.utils.imds.detect.provider", return_value="aws"):
...
def test_config_wo_status_config():
...
def test_config_w_name_pattern():
