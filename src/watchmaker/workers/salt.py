...
class SaltBase(ManagerBase):
    ...
    def _prepare_for_install(self):
        ...
        self.ent_env = self.config['ent_env']
        self.is_s3_bucket = self.config['is_s3_bucket']
    ...
    def process_states(self, states, this_log):
        ...
        if 'none' == self.config['salt_states'].lower():
            ...
            if 'highstate' == self.config['salt_states'].lower():
                ...
                cmd = ['state.sls', self.config['salt_states']]
                cmd.extend(self.salt_call_args)
                self.run_salt(cmd)
