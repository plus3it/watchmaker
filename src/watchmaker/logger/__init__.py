...
def prepare_logging(log_dir, log_level):
    ...
    """
    logformat = "%(asctime)s [%(name)s][%(levelname)-5s][%(process)s]: %(message)s"
...
def _configure_ec2_config_event_log():
    ...
    for event in events:
        if (
            event.find("ErrorType").text in MESSAGE_TYPES
            and event.find("Category").text == "Application"
            and event.find("AppName").text == "Watchmaker"
        ):
...
def _configure_ec2_launch_event_log():
    ...
    for event in events:
        if (
            event.get("level") in MESSAGE_TYPES
            and event.get("logName") == "Application"
            and event.get("source") == "Watchmaker"
        ):
